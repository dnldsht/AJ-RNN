        ...
        for i in range(Epoch):
            ...
            for input, prediction_target, mask, label_target, _, batch_need_label in utils.next_batch(config.batch_size, train_data, train_label, True, config.input_dimension_size, config.num_steps, Trainable=True):
                for _ in range(config.D_epoch):
                    ...
                    # D_solver adam op
                    # D_loss tensor Mean:0
                    # predict_M tensor Reshape_4:0
                    # M tensor Reshape_1:0
                ...
                for _ in range(config.G_epoch):
                    # loss_tensors['loss'] Tensor("loss_total/add_1:0")
                    # G_loss tensor Mean_1:0
                    # accuracy tensor accuracy:0
                    # G_solver Adam_1 op
                    # prediction tensor Reshape:0
                    # Last_hidden_output Tensor("Generator_LSTM/RNN/stacked_rnn_cells_269/gru_cell/add_3:0"

                    batch_loss, batch_g_loss, batch_accuracy, _, batch_train_Pre, batch_train_hidden = sess.run(
                        [loss_tensors['loss'], G_loss, accuracy,
                            G_solver, prediction, Last_hidden_output],
                        feed_dict={
                            input_tensors['input']: input,
                            input_tensors['prediction_target']: prediction_target,
                            input_tensors['mask']: mask,
